...
extern"C" void %(C_prefix)sget_gggh_tensor_coefs_ew_(const double *pInput,
	...
	if (!%(C_prefix)sexists_ew(mathematica_input_file)) {
	   std::cerr<<"Could Not find 'mathematica_input.fifo'. Place it somewhere as defined in fortran_bridge_ggHg_EW_fifo.cpp"<<std::endl;
       exit (EXIT_FAILURE);
	}

	string tmp_file = "%(path_prefix)s/mathematicaRoutines/"+%(C_prefix)suuid::%(C_prefix)sgenerate_uuid_v4()+"_tmp.fifo";
