...
set multiplot
set label "Higgs p_T" font ",13" at graph 0.4, graph 0.95
set xrange [0.0000e+00:1.7500e+02]
set bmargin 0 
set tmargin 0
set xtics nomirror
set ytics nomirror
...
set format y '10^{%T}'
set yrange [1.5057e-03:1.2372e+01]
...
plot \
'higgs_pt.HwU' index 0 using (($1+$2)/2):($3 >= 0 ? sqrt(-1) : abs($3)):4 w yerrorbar ls 101 title '',\
'higgs_pt.HwU' index 0 using (($1+$2)/2):($3 >= 0 ? sqrt(-1) : abs($3)) ls 101 title '',\
'higgs_pt.HwU' index 0 using (($1+$2)/2):($3 < 0 ? sqrt(-1) : $3):4 w yerrorbar ls 1 title '',\
'higgs_pt.HwU' index 0 using (($1+$2)/2):($3 < 0 ? sqrt(-1) : $3) ls 1 title titleHEFT,\
'higgs_pt.HwU' index 1 using (($1+$2)/2):($3 >= 0 ? sqrt(-1) : abs($3)):4 w yerrorbar ls 102 title '',\
'higgs_pt.HwU' index 1 using (($1+$2)/2):($3 >= 0 ? sqrt(-1) : abs($3)) ls 102 title '',\
'higgs_pt.HwU' index 1 using (($1+$2)/2):($3 < 0 ? sqrt(-1) : $3):4 w yerrorbar ls 2 title '',\
'higgs_pt.HwU' index 1 using (($1+$2)/2):($3 < 0 ? sqrt(-1) : $3) ls 2 title titleEW
#-- rendering subhistograms 'Relative scale and PDF uncertainty'
#unset label
#unset format
#set yrange [-1.5429e-01:1.4595e-01]
...
#plot \
#0.0 ls 999 title '',\
#'higgs_pt.HwU' index 1 using (($1+$2)/2):(0.0):(safe($4,$3,0.0)) w yerrorbar ls 2 title '',\
#'higgs_pt.HwU' index 0 using (($1+$2)/2):(0.0):(safe($4,$3,0.0)) w yerrorbar ls 1 title ''
#-- rendering subhistograms '(PURENLO|#2)/(PURENLOHEFT|#1 central value) ratio'
unset label
unset format
set yrange [0.015:5.4e-02]
set origin 0.0000e+00, 3.5000e-01
set size 1.0000e+00, 1.5000e-01
set mytics 2
set ytics auto
...
unset logscale y
unset ylabel
#set ylabel "ratio w.r.t. HEFT\\@NLOQCD"
set xlabel "[GeV]" offset 40,1.5,0
set label "EW \\@ NLO QCD / HEFT \\@ NLO QCD" font ",9" at graph 0.03, graph 0.13
plot \
1.0 ls 999 title '',\
'higgs_pt.HwU' index 2 using (($1+$2)/2):3:4 w yerrorbar ls 2 title '',\
...
################################################################################
unset multiplot
!ps2pdf "paper_higgs_pt.ps" &> /dev/null
!open "paper_higgs_pt.pdf" &> /dev/null
