...
extern"C" void %(C_prefix)sget_pphj_gghg_tensor_coefs_(
	...
	if (!%(C_prefix)spphj_gghg_exists(mathematica_input_file)) {
	   std::cerr<<"Could not find '"<<mathematica_input_file<<"'. Place it somewhere as defined in fortran_bridge_pphj_gghg_fifo.cpp"<<std::endl;
       exit (EXIT_FAILURE);
	}

	string tmp_file;
	if (pphj_batch_id < -1) {
		tmp_file = "%(path_prefix)s/mathematicaRoutines/"+%(C_prefix)spphj_gghg_uuid::%(C_prefix)sgenerate_uuid_v4()+"_tmp.fifo";
	} else if (pphj_batch_id == -1) {
		tmp_file = "/tmp/"+%(C_prefix)spphj_gghg_uuid::%(C_prefix)sgenerate_uuid_v4()+"_tmp.fifo";
	} else {
		tmp_file = "/tmp/batch_"+std::to_string(pphj_batch_id)+"/"+%(C_prefix)spphj_gghg_uuid::%(C_prefix)sgenerate_uuid_v4()+"_tmp.fifo";
	}
